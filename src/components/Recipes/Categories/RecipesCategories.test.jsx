...
describe('RecipesCategories', () => {
  it('renders with provided categories', () => {
    ...
    categories.forEach((category) => {
      ...
      expect(screen.getByRole('img', { name: category.name })).toHaveAttribute('src', category.image);
    });
  });
  ...
  it('renders with default categories when no categories are provided', () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      expect(screen.getAllByRole('img', {name: 'Category Placeholder'})[i]).toHaveAttribute('src', 'https://placeholder.co/150');
    }
  });
});